...
if (lang === 'en') {
  const text1BorderLoader = new THREE.TextureLoader().load(
    '../img/eng/Accounts_link.png'
  )
  const text1Border = new THREE.SpriteMaterial({ map: text1BorderLoader })
  myText1 = new THREE.Sprite(text1Border)
  const text2BorderLoader = new THREE.TextureLoader().load(
    '../img/eng/Licenses_link.png'
  )
  const text2Border = new THREE.SpriteMaterial({ map: text2BorderLoader })
  myText2 = new THREE.Sprite(text2Border)
  const text3BorderLoader = new THREE.TextureLoader().load(
    '../img/eng/Services_link.png'
  )
...
// cube12
loader.load('../img/money.png', function (texture) {
...
// cube14
loader.load('../img/ribbon.png', function (texture) {
  texture.anisotropy = renderer.getMaxAnisotropy()
  const material14 = [
    new THREE.MeshPhongMaterial({
      color: 0xb71e45,
...
// cube10
loader.load('../img/accept.png', function (texture) {
  texture.anisotropy = renderer.getMaxAnisotropy()
  const material10 = [
    new THREE.MeshPhongMaterial({
      color: 0xb71e45,
...
// cube15
loader.load('../img/payment.png', function (texture) {
  texture.anisotropy = renderer.getMaxAnisotropy()
  const material15 = [
    new THREE.MeshPhongMaterial({ map: texture }),
    new THREE.MeshPhongMaterial({
...
// cube24
loader.load('../img/youtube.png', function (texture) {
  texture.anisotropy = renderer.getMaxAnisotropy()
  const material24 = [
    new THREE.MeshPhongMaterial({ map: texture }),
    new THREE.MeshPhongMaterial({
...
// cube8
loader.load('../img/location.png', function (texture) {
  texture.anisotropy = renderer.getMaxAnisotropy()
  const material8 = [
    new THREE.MeshPhongMaterial({ map: texture }),
    new THREE.MeshPhongMaterial({
  ...
  new TWEEN.Tween(group[0].rotation)
    ...
    .onComplete(function () {
      ...
      group[0].children.forEach(el => {
        ...
        pos.push(
          Object.assign({}, el.getWorldPosition(target), { name: el.name })
        )
      })
      for (let i = 0; i < 9; i++) {
